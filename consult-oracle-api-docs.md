...
Make a request without payment → you will receive `402 Payment Required` with payment details.
{% endstep %}
...
1. Call `/analyze` without payment → receive 402 with payment details
...
2. User signs message via wallet popup (ed25519) and you base64 encode the signature.
{% endstep %}
